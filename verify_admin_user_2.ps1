...
Write-Output "Admin Created: $adminUserJson"
...
try {
    ...
    Write-Output "FAILURE: Weak password accepted (Create)!"
} catch {
    Write-Output "SUCCESS: Weak password rejected (Create). Status: $($_.Exception.Response.StatusCode.value__)"
}
...
try {
    ...
    Write-Output "FAILURE: Mismatch password accepted (Create)!"
} catch {
    Write-Output "SUCCESS: Mismatch password rejected (Create). Status: $($_.Exception.Response.StatusCode.value__)"
}
...
try {
    ...
    Write-Output "SUCCESS: Strong user created. ID: $($user.id)"
    ...
    try {
        ...
        Write-Output "FAILURE: Weak password accepted (Update)!"
    } catch {
        Write-Output "SUCCESS: Weak password rejected (Update)."
    }

} catch {
    Write-Output "FAILURE: Strong user creation failed! $($_.Exception.Message)"
